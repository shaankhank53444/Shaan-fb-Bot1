...
// --- START OF ORIGINAL OBFUSCATED CODE (UNCHANGED) ---
...
module.exports = {
  config: {
    ...
    description: "Talk to AI with Smart Intent Sensing",
  ...
  run: async function({ api, message, args, client }) {
    ...
    if (!args.length) {
      try {
        ...
        return api.sendMessage({ body: `🥀${userName}😗, ${randomReply}`, mentions: [{ tag: userName, id: senderID }] }, threadID, messageID);
      } catch (e) { return api.sendMessage("❌ Error sending AI reply.", threadID, messageID); }
    }
    ...
    // --- 🤖 SMART INTENT LOGIC ---
    // Bot sirf tabhi gana/video dhoondhega jab user "waqai" maange.
    ...
    // Agar dono types ke words milte hain (Irada + Item) tabhi action hoga
